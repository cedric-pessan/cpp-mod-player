   ...
   namespace wav
     {
        namespace
          {
             ...
             template<int FACTOR>
               auto buildResamplePositiveIntegerFactor(WavBitsPerSample bitsPerSample, mods::converters::Converter::ptr src) -> mods::converters::Converter::ptr
                 {
        ...
        // static
        auto WavConverterFactory::buildConverter(const mods::utils::RBuffer<u8>& data, const Format& format, StatCollector* statCollector, double peak) -> mods::converters::Converter::ptr
          {
             ...
             */
             ...
             std::vector<Converter::ptr> uncompressedChannels;
             ...
             switch(codec)
               {
                case WavAudioFormat::PCM:
                  if(uncompressedBitsPerContainer == WavBitsPerContainer::_8)
                    {
                       uncompressedChannels.push_back(std::make_unique<UnsignedToSignedConverter<u8>>(std::move(inputStream)));
                    }
                  else
                    {
		  ...
		  break;
             ...
             std::vector<Converter::ptr> demuxedChannels;
             ...
             std::vector<Converter::ptr> unpackedContainerChannels;
             ...
             std::vector<Converter::ptr> unpackedSampleChannels;
             ...
             std::vector<Converter::ptr> upscaledChannels;
                  ...
                  if(isResamplableByPositiveIntegerFactor(frequency) && peak == 1.0)
                    {
                       ...
                       for(size_t i=0; i < nbChannels; ++i)
                         {
                            upscaledChannels.push_back(std::make_unique<UpscaleConverter<s16, s8>>(std::move(unpackedSampleChannels[i])));
                         }
                    }
                  else
                    {
             ...
             if(peak != 1.0)
               {
                  std::vector<Converter::ptr> peakVec = std::move(upscaledChannels);
             ...
             std::vector<Converter::ptr> resampledChannels;
		  ...
		  for(size_t i = 0; i < nbChannels; ++i)
		    {
             ...
             // mixing stage
             converters::Converter::ptr mixedLeft;
             converters::Converter::ptr mixedRight;
             ...
             if(channelMask == 0)
                  ...
                  switch(nbChannels)
                    {
                     case 1:
                         {
                            auto duplicator = std::make_unique<ChannelCopyConverter>(std::move(resampledChannels[0]));
               ...
               {
                  std::vector<Converter::ptr> floatChannels;
             ...
             // downscale stage
             mods::converters::Converter::ptr downScaledLeft;
             mods::converters::Converter::ptr downScaledRight;
             ...
             switch(resampledBitsPerSample)
               {
                ...
                case WavBitsPerSample::_32:
                  downScaledLeft = std::make_unique<DownscaleConverter<s16, s32>>(std::move(mixedLeft));
                  downScaledRight = std::make_unique<DownscaleConverter<s16, s32>>(std::move(mixedRight));
                  break;
             ...
             // mux stage
             auto mux = std::make_unique<MultiplexerConverter>(std::move(downScaledLeft), std::move(downScaledRight));
             return mux;
          }
