...
namespace mods
{
   namespace wav
     {
	GenericResampleConverter::GenericResampleConverter(WavConverter::ptr src)
            : _src(std::move(src))/*,
	...
	bool GenericResampleConverter::isFinished() const
	     ...
	     std::cout << "GenericResampleConverter::isFinished() const" << std::endl;
	     return false;
          }
	...
	void GenericResampleConverter::read(mods::utils::RWBuffer<u8>* buf, int len)
	  {
	     ...
	     _src->read(buf, len);
	     std::cout << "GenericResampleConverter::read(mods::utils::RWBuffer<u8>*, int) const" << std::endl;
	  }
