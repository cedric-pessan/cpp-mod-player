   ...
   namespace converters
     {
        namespace impl
          {
             InternalCopySourceConverter::InternalCopySourceConverter(Converter::ptr src)
               : _src(std::move(src))
                 {
                 }
             ...
             ChannelCopyConverterSlave::ChannelCopyConverterSlave(InternalCopySourceConverter::sptr src, CopyDestId id)
               : _src(std::move(src)),
               _id(id)
                 {
                 }
             
             auto ChannelCopyConverterSlave::buildSlave() const -> Converter::ptr
               {
                  class make_unique_enabler : public ChannelCopyConverterSlave
                    {
                     public:
             ...
             auto ChannelCopyConverterSlave::isFinished() const -> bool
               {
                  return _src->isFinished(_id);
               }
             
             void ChannelCopyConverterSlave::read(mods::utils::RWBuffer<u8>* buf, size_t len)
               {
                  _src->read(buf, len, _id);
               }
          } // namespace impl
        
        ChannelCopyConverter::ChannelCopyConverter(Converter::ptr src)
          : ChannelCopyConverterSlave(std::make_unique<impl::InternalCopySourceConverter>(std::move(src)), impl::CopyDestId::MASTER),
          _copy(buildSlave())
            {
            }
        
        auto ChannelCopyConverter::getCopy() -> Converter::ptr
          {
